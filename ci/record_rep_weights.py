...
parser.add_argument("--rpc", help="node rpc host:port", default="http://[::1]:2225")
...
with open(args.output, 'wb') as of:
    ...
    for rep in reps:
        if rep["weight"] == 0:
            break
        acc_val = long(hexlify(b32decode(rep["account"].encode("utf-8").replace("bdm_", "").translate(tbl) + "====")), 16)
