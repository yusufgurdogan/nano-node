...
TEST (confirmation_height, gap_bootstrap)
{
	...
	auto send3 (std::make_shared<nano::state_block> (nano::genesis_account, send2->hash (), nano::genesis_account, nano::genesis_amount - 3 * nano::Gbdm_ratio, destination.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0));
...
TEST (confirmation_height, all_block_types)
{
	...
	nano::keypair key2;
	auto & store = node->store;
	nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::Gbdm_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (latest));
	nano::send_block send1 (send.hash (), key2.pub, nano::genesis_amount - nano::Gbdm_ratio * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send.hash ()));

	nano::open_block open (send.hash (), nano::test_genesis_key.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub));
	nano::state_block state_open (key2.pub, 0, 0, nano::Gbdm_ratio, send1.hash (), key2.prv, key2.pub, *system.work.generate (key2.pub));

	nano::send_block send2 (open.hash (), key2.pub, 0, key1.prv, key1.pub, *system.work.generate (open.hash ()));
	nano::state_block state_receive (key2.pub, state_open.hash (), 0, nano::Gbdm_ratio * 2, send2.hash (), key2.prv, key2.pub, *system.work.generate (state_open.hash ()));

	nano::state_block state_send (key2.pub, state_receive.hash (), 0, nano::Gbdm_ratio, key1.pub, key2.prv, key2.pub, *system.work.generate (state_receive.hash ()));
	...
	nano::state_block state_change (key2.pub, state_send.hash (), nano::test_genesis_key.pub, nano::Gbdm_ratio, 0, key2.prv, key2.pub, *system.work.generate (state_send.hash ()));

	nano::state_block epoch (key2.pub, state_change.hash (), nano::test_genesis_key.pub, nano::Gbdm_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (state_change.hash ()));

	nano::state_block epoch1 (key1.pub, change.hash (), key2.pub, nano::Gbdm_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (change.hash ()));
	nano::state_block state_send1 (key1.pub, epoch1.hash (), 0, nano::Gbdm_ratio - 1, key2.pub, key1.prv, key1.pub, *system.work.generate (epoch1.hash ()));
	nano::state_block state_receive2 (key2.pub, epoch.hash (), 0, nano::Gbdm_ratio + 1, state_send1.hash (), key2.prv, key2.pub, *system.work.generate (epoch.hash ()));

	auto state_send2 = std::make_shared<nano::state_block> (key2.pub, state_receive2.hash (), 0, nano::Gbdm_ratio, key1.pub, key2.prv, key2.pub, *system.work.generate (state_receive2.hash ()));
	nano::state_block state_send3 (key2.pub, state_send2->hash (), 0, nano::Gbdm_ratio - 1, key1.pub, key2.prv, key2.pub, *system.work.generate (state_send2->hash ()));

	nano::state_block state_send4 (key1.pub, state_send1.hash (), 0, nano::Gbdm_ratio - 2, nano::test_genesis_key.pub, key1.prv, key1.pub, *system.work.generate (state_send1.hash ()));
	nano::state_block state_receive3 (nano::genesis_account, send1.hash (), nano::genesis_account, nano::genesis_amount - nano::Gbdm_ratio * 2 + 1, state_send4.hash (), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send1.hash ()));
...
TEST (confirmation_height, modified_chain)
{
	...
	nano::keypair key1;
	auto & store = node->store;
	auto send = std::make_shared<nano::send_block> (latest, key1.pub, nano::genesis_amount - nano::Gbdm_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (latest));
...
TEST (confirmation_height, pending_observer_callbacks)
{
	...
	nano::keypair key1;
	nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::Gbdm_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (latest));
	auto send1 = std::make_shared<nano::send_block> (send.hash (), key1.pub, nano::genesis_amount - nano::Gbdm_ratio * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send.hash ()));
...
TEST (confirmation_height, frontiers_confirmation_mode)
{
	...
	// Always mode
	{
		...
		auto node = system.add_node (node_config);
		nano::state_block send (nano::test_genesis_key.pub, genesis.hash (), nano::test_genesis_key.pub, nano::genesis_amount - nano::Gbdm_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *node->work_generate_blocking (genesis.hash ()));
		{
			auto transaction = node->store.tx_begin_write ();
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
		}
...
TEST (confirmation_height, dependent_election)
{
	...
	auto send = std::make_shared<nano::send_block> (latest, key1.pub, nano::genesis_amount - nano::Gbdm_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (latest));
	auto send1 = std::make_shared<nano::send_block> (send->hash (), key1.pub, nano::genesis_amount - nano::Gbdm_ratio * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send->hash ()));
	auto send2 = std::make_shared<nano::send_block> (send1->hash (), key1.pub, nano::genesis_amount - nano::Gbdm_ratio * 3, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send1->hash ()));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send).code);
		ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send1).code);
